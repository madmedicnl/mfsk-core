...
/// 2. **CB**: the 11 m identifiers [`is_cb_callsign`] accepts — they have
///    no amateur structure to validate against, so they are admitted as a
///    callsign outright and travel on-air through the free-text and
///    hash-carrying layouts, as they do in WSJT-CB.
...
/// - **CB identifiers** ([`is_cb_callsign`]) pass without the allowlist —
///   they have no ITU amateur prefix to check, and their numeric-plus-numeric
///   structure is distinctive enough not to be a faulty-CRC landing spot.
...
mod tests {
    ...
    fn cb_callsigns_are_valid_and_plausible() {
        for &(call, accepted) in WSJT_CB_TABLE.iter().filter(|(_, a)| *a) {
            assert!(is_valid_callsign(call), "should accept {call} as a callsign");
            assert!(is_plausible_callsign(call), "should pass plausible for {call}");
        }
    }
